"""
Driven MHD turbulence test setup in astronomix (paper version).

Runs an isothermal, magnetized, periodic-box driven-turbulence sweep over a
range of resolutions with Ornstein-Uhlenbeck forcing, saving per-resolution
spectra and final states, then produces the paper figures: a resolution
comparison of the kinetic and magnetic energy spectra
(``turbulence_spectra_comparison.png``) and the final density slices
(``turbulence_final_density_N{N}.png``).
"""
...
# figures are written to the local figures/ directory, data to the local data/ directory
...
# -------------------------------------------------------------
# =============== ↓ Turbulence resolution sweep ↓ =============
# -------------------------------------------------------------
def run_turbulence_tests():
    # paper: [32, 64, 128, 256, 512]
...
def compare_spectra():
    ...
    num_cell_list = [32, 64] # paper: [32, 64, 128, 256]
