...
Run from the repo root:
    ...
    PYTHONPATH=$(git rev-parse --show-toplevel) python examples/scripts/forward/self_gravity/radial_profiles_comparison.py
...
# before any array is created, hence before jax.numpy is imported below. Native
# backend (the Pallas kernel runs reduced precision); see _collapse._backend_kwargs.
...
def run_and_cache():
    """Run each scheme to ``T_END`` and cache the final radial profiles.

    The density, (inward) radial velocity and entropy ``P / rho**gamma`` of the
    collapsed state are flattened to per-cell point clouds against radius and
    written to ``DATA_FILE`` (compressed, float32).
    """
    save_kwargs = {"resolution": np.array(RESOLUTION)}
    radius_saved = False
    for scheme in SCHEMES:
        print(f"  {scheme.label} (N={RESOLUTION}, {_PREC_LABEL})", flush=True)
        snapshots, helper_data, registered_variables = run_collapse(
            RESOLUTION,
        ...
        # The radius is scheme-independent, so it is stored only once.
        if not radius_saved:
        ...
        entropy = final_state[registered_variables.pressure_index] / density**GAMMA

        version = scheme.self_gravity_version
        save_kwargs[f"density_{version}"] = (
            np.asarray(density).ravel().astype(np.float32)
        )
        save_kwargs[f"v_r_{version}"] = np.asarray(v_r).ravel().astype(np.float32)
        save_kwargs[f"entropy_{version}"] = (
            np.asarray(entropy).ravel().astype(np.float32)
        )
...
def plot():
    """Draw the three-panel (density / radial velocity / entropy) figure."""
