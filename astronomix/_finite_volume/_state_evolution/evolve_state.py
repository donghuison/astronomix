...
def _evolve_gas_state_unsplit(
    ...
    if config.time_integrator == RK2_SSP:
        ...
        # # ALM Hyperparameters
        # # ==========================================
        # num_outer_iterations = 5   # Multiplier updates
        # num_inner_iterations = 10  # Gradient steps per multiplier update
        # initial_mu_energy = 0.0    # Start with 0 multiplier force
        # # Setting rho to 2e4 makes (rho/2)*C^2 equivalent to your original 1e4*C^2 penalty
        # initial_rho_energy = 20000.0 

        # def alm_loss_fn(final_primitive_state, mu_energy, rho_energy):
        #     # --- 1. PDE Consistency (The Objective to minimize) ---
        ...
        #     # Base objective (kept at 1e4 scale to match your original learning rate dynamics)
        #     consistency_loss = 1e4 * jnp.mean((conserved_state_final - mid_point_prediction_conserved) ** 2)

        #     # --- 2. Energy Constraint (C(u) = 0) ---
        ...
        #         registered_variables,
        #     )
        #     energy_constraint = final_energy - initial_energy  # C(u)

        #     # --- 3. Augmented Lagrangian Terms ---
        #     # L_A = Objective + mu * C(u) + (rho / 2) * C(u)^2
        #     alm_energy_term = mu_energy * energy_constraint + 0.5 * rho_energy * (energy_constraint ** 2)

        #     # --- 4. Positivity Constraints ---
        #     # Since you use hard projection (.set(jnp.maximum(...))) in the loop, 
        #     # leaving this soft penalty is fine and aids the gradient landscape.
        ...
        #     total_loss = consistency_loss + alm_energy_term + density_loss + pressure_loss
            
        #     # Return total_loss for grad calculation, and energy_constraint for multiplier update
        #     return total_loss, energy_constraint
        ...
        # def outer_step(outer_i, outer_state):
        #     # Unpack the ALM state
        #     p_state, mu_energy, rho_energy = outer_state
            
        #     # --- Inner Loop: Optimize Primal State ---
        #     def inner_step(inner_i, inner_p_state):
        #         # Calculate gradients only with respect to the primitive_state (arg 0)
        #         loss_eval_fn = lambda s: alm_loss_fn(s, mu_energy, rho_energy)[0]
        #         loss, grad = jax.value_and_grad(loss_eval_fn)(inner_p_state)
                
        #         # Gradient descent step
        #         inner_p_state = inner_p_state - 1e-3 * grad
                
        #         # Hard Projections for Positivity (Best practice for inequalities)
        #         inner_p_state = inner_p_state.at[registered_variables.density_index].set(
        #             jnp.maximum(inner_p_state[registered_variables.density_index], 1e-4)
        #         )
        #         inner_p_state = inner_p_state.at[registered_variables.pressure_index].set(
        #             jnp.maximum(inner_p_state[registered_variables.pressure_index], 1e-4)
        #         )

        #         inner_p_state = _boundary_handler(inner_p_state, config)
        #         return inner_p_state

        #     # Run the inner loop
        #     p_state = jax.lax.fori_loop(0, num_inner_iterations, inner_step, p_state)
            
        #     # --- Dual Update: Update the Lagrange Multiplier ---
        #     _, current_energy_constraint = alm_loss_fn(p_state, mu_energy, rho_energy)
            
        #     # The magic of ALM: mu_{k+1} = mu_k + rho * C(u)
        #     mu_energy_new = mu_energy + rho_energy * current_energy_constraint
            
        #     # Optional: You can increase rho slowly if convergence is stubborn (e.g. rho * 1.2),
        #     # but for stable fluid dynamics, a constant rho is usually preferred.
        #     rho_energy_new = rho_energy 

        #     # jax.debug.print("Outer {i} | Multiplier: {m} | Energy Error: {e}", 
        #     #                 i=outer_i, m=mu_energy_new, e=current_energy_constraint)

        #     return (p_state, mu_energy_new, rho_energy_new)

        # # Initialize the ALM state tuple
        # alm_state = (primitive_state, jnp.float32(initial_mu_energy), jnp.float32(initial_rho_energy))

        # # Run the outer loop
        # final_alm_state = jax.lax.fori_loop(0, num_outer_iterations, outer_step, alm_state)

        # # Extract the final physically strictly-conservative state
        # primitive_state = final_alm_state[0]
